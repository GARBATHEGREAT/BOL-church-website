...
export async function createAdminSession(email:string){const payload=b64(JSON.stringify({email:email.toLowerCase(),expires:Date.now()+SESSION_SECONDS*1000}));return payload+'.'+await signature(payload)}
export async function getAdminEmail(requestHeaders?:Headers){if(process.env.NODE_ENV==='development')return ADMIN_EMAIL;const cookie=(requestHeaders||await headers()).get('cookie')||'',token=cookie.split(';').map(x=>x.trim()).find(x=>x.startsWith(COOKIE+'='))?.slice(COOKIE.length+1);if(!token||!env.ADMIN_SESSION_SECRET)return null;const[payload,sig]=token.split('.');if(!payload||!sig||!safeEqual(await signature(payload),sig))return null;try{const data=JSON.parse(unb64(payload));return data.expires>Date.now()&&typeof data.email==='string'?data.email:null}catch{return null}}
export async function isAdmin(requestHeaders?:Headers){return!!await getAdminEmail(requestHeaders)}
export function sessionCookie(token:string){return `${COOKIE}=${token}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${SESSION_SECONDS}`}
